...
const NATURAL_DISASTERS = [
  { name: "earthquake", chance: 0.0001, effect: (npcs) => {
      npcs.forEach(npc => {
        if (Math.random() < 0.3) {
          npc.health = Math.max(0, npc.health - 20);
          showFloatingText("Earthquake! 🌍", npc.root.position, "#FF4757", scene, camera, engine);
        }
      });
    },
    duration: 3.0
  },
  { name: "lightning_strike", chance: 0.00005, effect: (npcs) => {
      const target = npcs[Math.floor(Math.random() * npcs.length)];
      target.health = Math.max(0, target.health - 40);
      showFloatingText("Lightning! ⚡", target.root.position, "#FFD700", scene, camera, engine);
    },
    duration: 2.0
  },
  { name: "wildfire", chance: 0.00008, effect: (npcs) => {
      npcs.forEach(npc => {
        if (Math.random() < 0.2) {
          npc.health = Math.max(0, npc.health - 15);
          showFloatingText("Wildfire! 🔥", npc.root.position, "#FF4500", scene, camera, engine);
        }
      });
    },
    duration: 5.0
...
export function triggerNaturalDisaster(activeNPCs, scene, camera, engine) {
  const now = performance.now() / 1000;
  const disaster = NATURAL_DISASTERS.find(d => Math.random() < d.chance && !d.activeAt);

  if (disaster) {
    disaster.activeAt = now;
    disaster.effect(activeNPCs);
    showNotif(`Natural Disaster: ${disaster.name}!`, "warn");
    setTimeout(() => {
      if (disaster.activeAt === now) {
        disaster.activeAt = null;
      }
...
export function updateNPCs(deltaTime, activeNPCs, placedObjects, occupiedGrid, scene, camera, engine, removeObjectById) {
  ...
  triggerNaturalDisaster(activeNPCs, scene, camera, engine);

  activeNPCs.forEach((npc) => {
    if (npc.isDead) {
      if (Math.random() < 0.001) {
        // Respawn NPC after a delay
        setTimeout(() => {
          const spawnX = Math.floor(Math.random() * (BOUND_MAX - BOUND_MIN - 1)) + BOUND_MIN;
          const spawnZ = Math.floor(Math.random() * (BOUND_MAX - BOUND_MIN - 1)) + BOUND_MIN;
          const newNpc = createNpc(npc.id, scene, { x: spawnX + 0.5, z: spawnZ + 0.5 });
          const index = activeNPCs.indexOf(npc);
          if (index !== -1) {
            activeNPCs.splice(index, 1);
          }
          activeNPCs.push(newNpc);
          showNotif(`${newNpc.name} has respawned!`, "info");
        }, 10000);
      }
      return;
    }
    ...
    if (npc.health <= 0) {
      npc.isDead = true;
      npc.root.dispose();
      showNotif(`${npc.name} has died!`, "warn");
