    ...
    if (channelIds.length === 0) {
      ...
      voiceContainer.innerHTML = channelIds.map((cid) => {
        ...
        const avatars = groupMembers.slice(0, 6).map((m) => {
          const avatarUrl = m.avatar_url || 'logo-512.png';
          return `<img src="${avatarUrl}" class="w-7 h-7 rounded-full object-cover border-2 border-slate-900 -ml-2 first:ml-0" alt="${escapeHtml(m.username)}" title="${escapeHtml(m.username)}" onerror="this.src='logo-512.png'" />`;
        }).join('');
      ...
      return `
        <div class="flex items-center gap-3 p-3 rounded-2xl bg-slate-900/80 border border-slate-800/90 hover:border-indigo-500/40 transition">
          <div class="relative shrink-0">
            <img src="${avatarUrl}" class="w-10 h-10 rounded-full object-cover border border-slate-700/80" alt="${username}" onerror="this.src='logo-512.png'" />
